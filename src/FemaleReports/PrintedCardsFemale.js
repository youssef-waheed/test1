    ...
    if (ofYear) {
      try {
        const response = await axios.get(
          `http://localhost:5000/reports/printedFemalesCardsReport${queryString}`
        );
      ...
      try {
        const response = await axios.get(
          `http://localhost:5000/reports/printedFemalesCardsReport`
        );
        console.log(response);
        const { data } = response.data;
        setStudents(data.student);
...
export default PrintedCardsFemale;
