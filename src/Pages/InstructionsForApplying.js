...
const AdminPanel = () => {
  ...
  const uploadPdfFile = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/instructions/upload",
        formData,
        {
          headers: {
    ...
    try {
      const response = await fetch("http://localhost:5000/instructions", {
        ...
        headers: {
          ...
          Authorization:
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE3MDYyMDExNzIsImV4cCI6NDgzNjIyMDExNzJ9.iV6q_TeUygXxwsaad0jMMx9B_ZO8_ly6KeGMmNqnkHk", // Include the valid bearer token
        },
      ...
      if (response.ok && data.status === "success") {
        setInstructions([
          ...instructions,
          { _id: data.data._id, order: orderCounter, ...newInstruction },
        ]);
        setOrderCounter(orderCounter + 1);
        setEditingInstruction(null);
      } else {
        console.error(
          "Error adding instruction:",
  ...
  const saveChanges = async (updatedInstruction) => {
    ...
    try {
      await handleApiRequest(
        `http://localhost:5000/instructions/${editingInstruction._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      setInstructions(
        instructions.map((inst) =>
          inst._id === updatedInstruction._id
            ? { ...inst, ...updatedInstruction }
            : inst
        )
      );
      setEditingInstruction(null);
  ...
  return (
    <div>
      ...
      />
      <InstructionForm
        onSubmit={editingInstruction ? saveChanges : addInstruction}
        onCancel={cancelEdit}
        editedInstruction={editingInstruction}
      />
      <div>
        <input type="file" onChange={handleFileUpload} />
        {auth && (auth.athurity === "الكل" || auth.athurity === "ادخال") && (
          <button style={{ backgroundColor: "green" }} onClick={uploadPdfFile}>
            Upload PDF
          </button>
        )}
