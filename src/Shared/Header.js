...
function Content({ activeIndex, activeTab, show }) {
  ...
  return (
    show &&
    <div>
      {buttonContent[activeTab][activeIndex]}
    </div>
  )
}
    ...
    <div>
      ...
      <div id="contentDiv">
        {buttonSets[activeTab].map((btn, index) =>
          <button key={index} onClick={() => handleButtonClick(index)}>{btn}</button>
        )}
        <div id="content">
          
          <Content activeIndex={activeIndex} activeTab={activeTab} show={show} />
        </div>
      </div>
    </div>
  );
